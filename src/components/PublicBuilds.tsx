    ...
    <>
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {builds.map((build) => {
          ...
          const caseItem = build.items.find(
            (item: any) => item.product.category === 'CASE'
          );
          const cpuItem = build.items.find(
            (item: any) => item.product.category === 'CPU'
          );
          const gpuItem = build.items.find(
            (item: any) => item.product.category === 'GPU'
          );
          const ramItem = build.items.find(
            (item: any) => item.product.category === 'RAM'
          );

          // Main image: Case > GPU > CPU
          const mainImage =
            caseItem?.product.imageUrl ||
            gpuItem?.product.imageUrl ||
            cpuItem?.product.imageUrl;
              ...
              <div className="p-6">
                ...
                <div className="flex items-center gap-4 mb-4 text-sm text-gray-600">
                  <div className="flex items-center gap-1">
                    ...
                    <span>
                      {build.averageRating ? build.averageRating.toFixed(1) : 'N/A'} (
                      {build.totalRatings})
                    </span>
                  </div>
